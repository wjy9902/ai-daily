"""The plan stage: one editor call over every judged candidate, and its gates."""
...
def _normalize_repository_plan(plan: EditorialPlan, events: list[Event]) -> EditorialPlan:
    ...
    selections = [
        selection.model_copy(
            update={
                "headline": repository_update_copy(selection.headline),
                "brief": repository_update_copy(selection.brief),
            }
        )
        if selection.event_id in repository_events
        else selection
    ...
    insights = [
        insight.model_copy(update={"text": repository_update_copy(insight.text)})
...
def _planning_instructions(config: PipelineConfig) -> str:
    return (
        ...
        "每条选题的 headline 和 brief 必须与该 event_id 的标题及证据主题一致，"
        "不能借一条对比报道、未来版本预告或旁支快讯写另一事件的发布稿。"
        "若决定报道同日不同产品的官方发布，须分别使用各自的官方事件；"
        "跨产品报道只可写其自身的对比主题，不可代替任一官方发布。"
        "同一条持续进行的新闻出现新进展时(额度再次重置、灰度扩大到新档位、"
        "价格再次调整、此前的传闻被官方确认)，这是新条目而不是重复，"
        "应当收录并在 brief 中写清这次的新变化是什么。"
        "读者是每天使用这些模型和工具的开发者与重度用户，"
        "选题以他们今天能感知到的变化为第一优先级，依次是："
        "一、额度、限流、用量重置与配额调整；"
        "二、定价调整、限免、折扣与免费额度开放；"
        "三、功能上线、灰度、内测开放与功能下线；"
        "四、真实产品与模型发布、能力变化、模型可用性与服务状态变化；"
        "五、广泛采用的开源工具与权重发布。"
        "上述之外的宏观产业新闻(财报、股价、关税、芯片制程、融资与并购、"
        "与 AI 产品无直接关系的诉讼与人事)属于次要选题，"
        "只有确实重大时才收录，且三个数组合计不超过 4 条。"
        "同等重要时，优先选择读者当天可以据此改变操作的消息，"
        "而不是只能作为背景知道的消息。"
        ...
        "在证据充足时兼顾国际一线实验室、开发者工具和中国 AI。"
        "headline 与 brief 使用简洁中文，brief 要同时说清发生了什么及为何值得关注；"
        "brief 应写清关键数字(参数量、价格、额度、生效时间、适用范围)，"
        "有这些数字时不要只写结论。"
    ...
    for selection in plan.selections:
        ...
        for field, value in (("headline", selection.headline), ("brief", selection.brief)):
            ...
            if (
                ...
                and has_repository_release_claim(value)
            ):
                raise ValueError(
...
def _validate_plan_evidence(plan: EditorialPlan, events_by_id: dict[str, Event]) -> None:
    ...
    for insight in plan.editor_viewpoint:
        ...
        if set(insight.evidence_ids) & repository_evidence and has_repository_release_claim(
            insight.text
        ):
